...
public class MainMap implements Serializable {
    ...
    public boolean[][] getProcessedEvents() {
        return eventProcessed;
    }

    public void loadProcessedEvents(boolean[][] events) {
        if (events != null && events.length == SIZE && events[0].length == SIZE) {
            this.eventProcessed = events;
        }
    }
    ...
    public void loadBoardState(char[][] boardState) {
        if (boardState != null && boardState.length == SIZE && boardState[0].length == SIZE) {
    ...
    public char[][] getBoard() {
        return this.board; // Возвращает текущее состояние карты
    }

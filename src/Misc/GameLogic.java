...
public class GameLogic {
    ...
    public Enemy enemy;
    private QuestHandler questHandler; // Добавляем поле
    ...
    public GameLogic(Player player, MainMap mainMap, BattleMap battleMap, Scanner scanner) {
        ...
        this.questHandler = new QuestHandler(mainMap);
    }
    ...
    public boolean movePlayerOnSettlementMap(char direction) {
        ...
        switch (cell) {
            case 'С': settlement.handleElderHouse(player, this.questHandler); break;
            case 'К': settlement.handleBlacksmith(player); break;
            case 'Т': settlement.handleTavern(player); break;
            case 'Д': settlement.handleQuestBoard(player); break;
        }
    ...
    public void handleEvent(Scanner scanner) {
        ...
        if (event.getEventBehavior() instanceof BattleEvent) {
            ((BattleEvent)event.getEventBehavior()).trigger(player);
            eventx = player.getX();
            eventy = player.getY();
        } else {
        ...
        mainMap.markEventAsProcessed(eventx, eventy);
        mainMap.placePlayer(eventx, eventy, player);
    }
    ...
    private EventTile createRandomEvent() {
        ...
        EventBehavior eventBehavior = random.nextBoolean() ?
                new BattleEvent(battleManager, scanner, battleMap):
